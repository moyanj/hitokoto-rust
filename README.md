...
- 🔌 多数据库支持：MySQL / SQLite
- 🚀 高性能：异步架构 + 编译时 SQL 优化
- 💡 内存优化模式：`--memory` 可将数据加载至内存数据库，进一步加速 3–10 倍(0.7.1+ 版本因未知问题可能导致性能下降)
...
```bash
target/release/hitokoto-rust -d sqlite://1.db -w 8
```
...
[官方服务器性能测试结果](https://github.com/hitokoto-osc/hitokoto-api#benchmark)
